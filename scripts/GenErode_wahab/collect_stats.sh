...
# awk 'NR>1 {print$1}' ../../../../../jselwyn/spratelloides_delicatulus_lcwgs/GenErode_20k/config/historical_samples.txt | cut -d "_" -f1 | sort | uniq > samp_list
...
## usage of the script
# sbatch collect_stats.sh

SAMPLES_FILE=/archive/carpenterlab/pire/marianne/spratelloides_delicatulus_lcwgs/QC/scripts/inputfiles/samp_list
...
mapped="results/historical/mapping/${ref}"
...
echo $SAMPLENAME $total_seqs $total_mapped $total_uniq $total_MQ25 $endogenous $complexity $grr $total_cov $read_length >> $outdir/historical_sequencing_stats.txt

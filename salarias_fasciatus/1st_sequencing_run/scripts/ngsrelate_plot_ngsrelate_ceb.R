...
#ceb hack
df <- read.table("../ngsRelate/out_ngsRelate",h=T)
...
df %>%
    mutate(era_a = case_when(a <= 31 ~ "A",
                           TRUE ~ "C"),
           era_b = case_when(b <= 31 ~ "A",
                             TRUE ~ "C"),
           era_comp = case_when(era_a == era_b & era_a == "A" ~ "AA",
                                era_a == era_b & era_a == "C" ~ "CC",
                                TRUE ~ "AC")) %>%
    ggplot(aes(x=nSites)) +
    geom_histogram() +
    facet_grid(era_comp ~ .)

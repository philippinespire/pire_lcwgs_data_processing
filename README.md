...
## Wahab best practices
_Marianne Dehasque Jul 31, 2026_

Wahab is the HPC cluster at ODU used to analyze data generated within the PIRE and Albatross Re-Collection project.

Here's the official wiki to the Wahab cluster: https://wiki.hpc.odu.edu/en/Cluster/Wahab

_Best Practices_

* Don't copy large files into new directories. Instead, use symbolic links. As a rule of thumb, if data is larger than 2Gb, use a symbolic link. This goes for any file or project, and overrides older instructions that tell you to do so.
* Use GitHub Pull and Push timely. You want to use GitHub pull before working on files within a shared repository and make sure you push your changes in a timely manner. Otherwise there might be conflicts and they can be a pain to fix.

_Tips and Tricks_

Get your own conda installation

## Workflow steps
The lcWGS pipeline is still under active development, even after 4 years! 

### 1. Set up your working directory on Wahab and link it to GitHub
Inside /archive/carpenterlab/pire, make a directory with your name (if it doesn't exist yet), eg
```
mkdir -p mpinsky
cd mpinsky
```

Create the repository for your analysis on GitHub (e.g., pire_chromis_viridis_lcwgs) and clone it to the your directory, eg:
```
git clone https://github.com/philippinespire/pire_chromis_viridis_lcwgs.git
```

### 2. Get a reference genome
<details><summary>2. Get a reference genome</summary>
<p>
  
Within your repository, make a new directory `refGenome`:
...
```
# cp /home/e1garcia/shotgun_PIRE/pire_ssl_data_processing/<species_name>/<best assembly>/scaffolds.fasta /archive/carpenterlab/pire/<yourname>/pire_<species_name>_lcwgs/refGenome/

cp /home/e1garcia/shotgun_PIRE/pire_ssl_data_processing/ostorhinchus_chrysopomus/SPAdes_allLibs_decontam_R1R2_noIsolate/scaffolds.fasta /archive/carpenterlab/pire/<yourname>/pire_ostorhinchus_chrysopomus_lcwgs/refGenome/
```
...
We use the [nf-trim-generode pipeline](https://github.com/philippinespire/nf-pipelines/tree/main/nf-trim-generode) to map historical and modern sequencing data against a reference.

### 5. Calculating diversity metrics
...
### 6. Performing selection scans

Step 3 (above) will run one selection scan. More options are under construction.

### 7. Uploading Species Data (GEOME, NCBI, Zenodo)
...
_Note that this section still has some paths that are out of date. If you see that, please edit them or let us know. The general idea is still correct!_
...
First, locate your genotype likelihood file. This will be a `*.beagle.gz` file generated in `results/` during the nf-angsd-selection pipeline.
```
(model)
...
## Steps to Process Your Data [archived]
